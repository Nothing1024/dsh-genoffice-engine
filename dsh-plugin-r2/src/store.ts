...
export type TabsState = { active: TabId }
 ...
 */
export function createTabsStore(): EngineStoreHandle<TabsState, TabsActions> {
  return defineStore({
    init: (): TabsState => ({ active: 'workspace' }),
    actions: {
      setActive: (draft, id: TabId) => { draft.active = id },
    },
  })
}
