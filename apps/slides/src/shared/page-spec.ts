 ...
 *
 * Image fetch and decode are injected so this module stays testable in
 * plain Node and importable from both Electron main and the web renderer.
 */
import {
  addElement,
  addPicture,
...
export interface BuildPageDeps {
  ...
  /** Decodes natural pixel size for cover-cropping; null skips the crop. May be async (browser Image / createImageBitmap). */
  imageDims?: (
    bytes: Uint8Array,
...
export async function buildPagePptx(
  ...
  for (const el of spec.elements) {
    ...
    if (el.type === 'image') {
      ...
      const dims = (await deps.imageDims?.(img.bytes)) ?? null
      if (dims) {
