...
/** Load the wasm bytes ourselves: the browser fetch through Vite's ?url asset */
export function loadPdfium(): Promise<Pdfium> {
  pdfiumPromise ??= (async () => {
    ...
    const raw = await fetchAssetBytes(PDFIUM_WASM_URL)
    // Exact slice: the fetched ArrayBuffer may be larger than the file
...
let fallbackFontBytes: Uint8Array | null | undefined

async function loadFallbackFont(): Promise<Uint8Array> {
  if (fallbackFontBytes === undefined) {
    try {
      fallbackFontBytes = await fetchAssetBytes(FALLBACK_FONT_URL)
    } catch {
...
async function loadEditFont(
  ...
  if (cached === undefined) {
    // browser: every family maps to the bundled LiberationSans face for that style
    cached = null
    const url = EDIT_FONT_URLS[style]
    if (url) {
      try {
        cached = await fetchAssetBytes(url)
      } catch {
        cached = null
      }
    }
...
/** Edit-font ids (browser: LiberationSans-backed; load failure → empty list) */
export async function listEditFonts(): Promise<string[]> {
  const ids: string[] = []
  for (const id of Object.keys(EDIT_FONT_PATHS)) {
    if ((await loadEditFont(id)) !== null) ids.push(id)
  }
  return ids
}
...
async function rebuildFontBytes(
  ...
  const fallback = asFontBuffer(await loadFallbackFont())
