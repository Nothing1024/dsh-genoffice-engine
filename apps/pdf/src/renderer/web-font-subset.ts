import { HB_SUBSET_WASM_URL } from './web-wasm-assets'
...
function loadHb(): Promise<HbSubset> {
  hbPromise ??= (async () => {
    const resp = await fetch(HB_SUBSET_WASM_URL)
    if (!resp.ok) throw new Error(`hb-subset.wasm fetch failed: ${resp.status}`)
