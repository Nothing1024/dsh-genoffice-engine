...
export function apply(ctx: ClientContext): void {
  // Duplicate-instance guard: the profile can transiently hold two rows for
  // this package (a live HMR rename leaks the old entry — its bundle stays
  // served while the renamed row also mounts). The older instance must stay
  // inert instead of crashing the single-slot registration (and the boot
  // sweep with it); the newer instance owns the sidebar.
